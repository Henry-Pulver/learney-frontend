import { buttonPress, createTipElement } from "./utils";
...
function createIntroHTML(introSlides) {
  ...
  let gif = createTipElement(
    "video",
    {
      class: "intro-gif",
      src: staticFileLocation + slideInfo.gif,
      autoplay: "",
      loop: "",
    },
    []
  ...
  for (let i = 0; i < slideInfo.text.length; i++) {
    ...
    if (typeof textItem == "string") {
      ...
      textDiv.appendChild(
        createTipElement(textItem[0], textItem[1], textItem[2])
      );
    }
  }
  ...
  if (introSlideNumber === 0) {
    ...
    prevSlide.onclick = buttonPress(
      changeSlideFunction(false, introSlides),
      "prev-intro"
    );
  }
  ...
  if (introSlideNumber + 1 === numSlides) {
    ...
    nextSlide.onclick = buttonPress(
      changeSlideFunction(true, introSlides),
      "next-intro"
    );
  }
  let slideNumDiv = createTipElement(
    "div",
    { class: "intro-slide-num-div" },
    (introSlideNumber + 1).toString() + "/" + numSlides.toString()
  );

  let slideDiv = createTipElement("div", { class: "intro-slide-change-div" }, [
    prevSlide,
    slideNumDiv,
    nextSlide,
  ]);
  let closeButton = createTipElement(
    "button",
    { class: "close", id: "close-intro" },
    "X"
  );
  closeButton.onclick = buttonPress(removeIntroTippy, "close-intro");

  let introContainer = createTipElement("div", { class: "intro-container" }, [
    title,
    gifDiv,
    textDiv,
    slideDiv,
  ]);
  return createTipElement("div", { class: "intro-content-container" }, [
    introContainer,
    closeButton,
  ]);
}
