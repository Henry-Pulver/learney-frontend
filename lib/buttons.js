import { initialiseGraphState, resetProgress } from "./learningAndPlanning";
import {
...
if (!isAnonymousUser(userId)) {
  document.getElementById("log-out").onclick = buttonPress(function () {
    location.href = "/logout";
  }, "log-out");
} else {
  document.getElementById("log-in").onclick = buttonPress(function () {
    location.href = "/login/auth0";
  }, "log-in");
}
document.getElementById("feedback-button").onclick = buttonPress(function () {
  window.open(
    "https://docs.google.com/forms/d/e/1FAIpQLSeWyrpKy0r4LbQuuHt5FIL9PYU7KFfLSxFnnuBDs3-zaofW7A/viewform",
    "_blank"
  );
}, "feedback-button");
document.getElementById("slack-button").onclick = buttonPress(function () {
  window.open(
...
if (allowSuggestions) {
  document.getElementById("make-suggestion").onclick = buttonPress(
    goToFormFunction("concept"),
    "make-suggestion"
  );
} else {
...
if (editMapEnabled) {
  ...
  document.getElementById("reset-progress").onclick = buttonPress(
    resetProgressButton,
    "reset-progress"
  );
}
...
// RESET PAN BUTTON
document.getElementById("reset-pan").onclick = buttonPress(function () {
...
// SAVE MAP BUTTON
if (editMapEnabled) {
  document.getElementById("save-layout").onclick = buttonPress(
    saveMap,
    "save-layout"
  );
} else {
...
if (editMapEnabled) {
  document.getElementById("run-dagre").onclick = buttonPress(
    autoGenerateLayout,
    "run-dagre"
  );
} else {
...
// RESET TO START
if (editMapEnabled) {
  document.getElementById("reset-layout").onclick = buttonPress(
    resetLayout,
    "reset-layout"
  );
} else {
