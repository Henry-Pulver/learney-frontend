...
if (isAnonymousUser(userId)) {
  ...
  signInTooltip.show();
} else if (localStorage.getItem(transferredToProfileOnce) === null) {
  // Deals with first-time signed in users
  localStorage.setItem(transferredToProfileOnce, "true");
} else {
  // Deals with other signed in users
  if (localStorage.getItem(deleteStoredProgress) === "true") {
    localStorage.removeItem(learnedNodesString);
    localStorage.removeItem(goalNodesString);
  }
...
export var learnedNodesPromise = Promise.resolve(
  initialiseFromStorage(learnedNodesString)
);
export var goalNodes;
export var goalNodesPromise = Promise.resolve(
  initialiseFromStorage(goalNodesString)
);
