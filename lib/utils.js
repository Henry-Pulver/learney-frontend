import { headers, jsonHeaders } from "./csrf";

// Get context from html
export const mapUUID = JSON.parse(
  document.getElementById("map_uuid").textContent
);
export const mapVersion = JSON.parse(
  document.getElementById("map_version").textContent
);
...
export const allowSuggestions = JSON.parse(
  document.getElementById("allow_suggestions").textContent
);
export const localStorage = window.localStorage;
var alreadyQuestionAnswerUser =
  localStorage.getItem("questionAnswerUser") === "true";
...
export function LightenDarkenColorByFactor(col, factor) {
  if (factor < 0) {
    console.error(
      `Factor given (${factor}) in LightenDarkenColorByFactor() cannot be negative!`
    );
  }
...
export function logPageView() {
  fetch("/api/v0/page_visit", {
    ...
    body: JSON.stringify({
      user_id: userId,
      page_extension: location.pathname,
    }),
  }).then(handleFetchResponses);
