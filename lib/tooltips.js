...
import { removeIntroTippy } from "./intro";
...
const staticFileLocation = document
  .getElementById("static-root")
...
function createLinkPreviewArray(nodeData, urls) {
  ...
  for (let i = 0; i < urls.length; i++) {
    ...
    link.onclick = function () {
      logContentClick(urls[i], nodeData.id);
    };
...
function createTooltipHTML(node) {
  ...
  let learnedSlider = createLearnedSlider(
    node,
    learned,
    onLearnedSliderClick(node),
    "I know this!"
  );
  let goalSlider = createLearnedSlider(
    node,
    goalSet,
    onSetGoalSliderClick(node),
    "Set goal"
  );
  ...
  if (urls.length > 0) {
    let linkList = createTipElement(
      "ol",
      { class: "tooltip-link" },
      createLinkPreviewArray(node.data(), urls)
    );
