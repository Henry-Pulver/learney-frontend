...
export function initCy(
  ...
  let positionsDefined =
    originalMapJSON.nodes.length > 0 &&
    originalMapJSON.nodes[0].position !== undefined;
  let layout;
  if (positionsDefined) {
    layout = presetLayout;
  } else {
...
function bindRouters(
  ...
  cy.on("tap pan zoom", function (e) {
    if (e.target === cy) {
      if (selectedNodeID !== Infinity)
        hideConceptTippy(cy.getElementById(selectedNodeID), userId, sessionId);
  ...
  if (editMap) {
    cy.on("tap", 'node[nodetype = "concept"]', handleEditNodeData);
    cy.on("tap", 'node[nodetype = "field"]', handleEditParentNodeData);
  } else {
