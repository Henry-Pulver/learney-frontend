import { LightenDarkenColorByFactor } from "./utils";
import { makeTippy, removeTippy } from "./tooltips";
import {
...
export function initCy(
  ...
  originalMapJSON = JSON.parse(mapJson);
  ...
  bindRouters(backendUrl, userId, mapUUID, sessionId);
...
export function dagreOnSubjects() {
  ...
  subjects.forEach(function (subject) {
    let subject_subgraph = cy.filter('node[subject = "' + subject + '"]');
...
function bindRouters(backendUrl, userId, mapUUID, sessionId) {
  ...
  cy.on("tap", 'node[nodetype = "concept"]', function (e) {
    ...
    if (!isAnimated) {
      ...
      fitCytoTo({ eles: concept.neighborhood(), padding: 50 }, function () {
        makeTippy(concept, backendUrl, userId, mapUUID, sessionId);
  ...
  cy.on("cxttap", 'node[nodetype = "concept"]', function (e) {
    onSetGoalSliderClick(e.target, backendUrl, userId, mapUUID, sessionId)();
  });
}
