...
export default function QuestionModal({
  ...
  const [answersGiven, setAnswersGiven] = useState(questionSet.map(() => null));
  const [currentQuestionIndex, setCurrentQuestionIndex] = useState(0);
  const nextQuestion = () => setCurrentQuestionIndex(currentQuestionIndex + 1);

  useEffect(() => {
    setCurrentQuestionIndex(0);
    setAnswersGiven((prevAnswersGiven) => questionSet.map(() => null));
  }, [questionSet]);

  const onAnswerClick = (answerText) => {
    ...
    fetch(`${backendUrl}/api/v0/question_response`, {
      method: "POST",
      headers: jsonHeaders,
      body: JSON.stringify({
        user_id: userId,
          ...
          >
            ...
            {currentQuestionIndex in questionSet && (  // <-- Crushes a rare bug
              <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full sm:p-6">
                ...
                {/*FEEBACK*/}
                {answersGiven[currentQuestionIndex] !== null && // Check that the question has been answered
