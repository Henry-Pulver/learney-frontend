...
describe("<Results>", () => {
  ...
  it("calls onClick when result is clicked", () => {
    ...
    const { container } = render(
      <Results {...defaultProps} onClick={onClick} />
    );
  ...
  it("hides the icon if showIcon is false", () => {
    const { container } = render(
      <Results {...defaultProps} showIcon={false} />
    );
  ...
  it("calls onHover when result is hovered", () => {
    ...
    const { container } = render(
      <Results {...defaultProps} onHover={onHover} />
    );
  ...
  it("calls formatResult when renders results", () => {
    ...
    const { container } = render(
      <Results {...defaultProps} formatResult={formatResult} />
    );
